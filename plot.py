...
def list_colormap(value_list, colormap="colorblind" if has_seaborn else "jet",
    sort=True):
    """
    ...
    colormap : matplotlib colormap or seaborn color palette
        Colormap or color palette to use. (default: "colorblind" if seaborn is
        available else "jet")
    sort : bool
    ...
    try:                # matplotlib colormap
        ...
        return {value: color
            for value, color in zip(
                value_list,
                seaborn.color_palette(colormap, len(value_list)))}
